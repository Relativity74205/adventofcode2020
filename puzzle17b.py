...
def perform_cycle(arr):
    ...
    arr_copy = arr.copy()
    for w in range(1, arr.shape[0]):
        for x in range(1, arr.shape[1]):
            for y in range(1, arr.shape[2]):
                for z in range(1, arr.shape[3]):
                    active_neighbors = arr[w - 1:w + 2, x - 1:x + 2, y - 1:y + 2, z - 1:z + 2].sum() - arr[w, x, y, z]
                    if arr[w, x, y, z]:
